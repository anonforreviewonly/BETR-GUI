...
try:
    from hypermapper import run  # noqa
except ModuleNotFoundError:
    print("Ignoring module hypermapper.")
...
class HypermapperOptimization:
    ...
    def __init__(
        ...
        exp_name: str = None,
        hotstart_with_previous_parameters: bool = False,
        random_forest: bool = False,
        feasibility: bool = False,
        batch_size: int = 1,
        new_parameter_defaults: bool = False,
        ...
        self.new_parameter_defaults = new_parameter_defaults
        self.extra_settings = extra_settings if isinstance(extra_settings, dict) else {}
        ...
        if self.params_json and self.num_params > 0:
    ...
    def _process_parameters(self):
        ...
        for pn in self.param_nodes:
            if pn.parameters is not None:
                ...
                for parameter_name, parameter in pn.parameters.items():
                    if parameter.data_type == ParameterTypes.FLOAT:
                        ...
                        if isinstance(parameter.value, list):
                            ...
                            for i in range(len(parameter.value)):
                                if self._add_real_param_with_list(self.params_json, self._param_str(pn, self.num_params, i), parameter, i):
                                    subparam_counter += 1
                            if subparam_counter > 0:
                                self.num_params += subparam_counter - 1
                            else:
                                continue
                        elif not self._add_real_param(self.params_json, self._param_str(pn, self.num_params), parameter):
                            continue
                    elif parameter.data_type == ParameterTypes.POSITION or parameter.data_type == ParameterTypes.XY_POSITION:
                        subparam_counter = 0
                        for i in range(len(parameter.value)):
                            if self._add_real_param_with_list(self.params_json, self._param_str(pn, self.num_params, i), parameter, i):
                                subparam_counter += 1
                        if subparam_counter > 0:
                            ...
                            continue
                    elif parameter.list_of_values is not None:
                        # If there is nothing to learn, 'value' needs to be set to the only value in the list
                        if len(parameter.list_of_values) == 1:
                            parameter.value = parameter.list_of_values[0]
                            continue
                        elif len(parameter.list_of_values) > 1:
                            # The "at pos" node has a special rule that "valueA at valueA" is not allowed, so the 2nd parameter needs to handled differently
                            if pn.name == "at pos":
                                if parameter_name == "target_object":
                                    name = self._param_str(pn, self.num_params)
                                    if self._add_categorical_param(self.params_json, name, parameter):
                                        at_first_param_name = name
                                    else:
                                        continue
                                elif parameter_name == "relative_object":
                                    if self._add_categorical_param(self.params_json, self._param_str(pn, self.num_params), parameter, excluded_np=pn.parameters["target_object"], excluded_param_name=at_first_param_name):
                                        at_first_param_name = None
                                    else:
                                        continue
                            else:
                                if not self._add_categorical_param(self.params_json, self._param_str(pn, self.num_params), parameter):
                                    continue
                    else:
                        continue
                    self.num_params += 1
    ...
    def _generate_json(self,
        ...
        doe["number_of_samples"] = max(5, int(1.0 * self.num_params))
        ...
        j["batch_size"] = self.batch_size
        j["new_parameter_defaults"] = self.new_parameter_defaults
        j["exploration_augmentation"] = self.extra_settings.get("exploration_augmentation", 0.0001)
        j["local_search_step_size"] = self.extra_settings.get("local_search_step_size", 0.1)
        if not self.random_forest:
            # Set a lengthscale priors for the prior expectation of the impact of each parameter
            j["lengthscale_prior"] = self.extra_settings.get("lengthscale_prior", {"name": "gamma", "parameters": [1.2, 0.2]})
            # Some tuning to make it run faster
            j["local_search_improvement_threshold"] = self.extra_settings.get("local_search_improvement_threshold", 1e-4)
        ...
        j["local_search_random_points"] = self.extra_settings.get("local_search_random_points", 100)
        j["local_search_starting_points"] = self.extra_settings.get("local_search_starting_points", 5)
    ...
    def _evaluate_single_function(
        ...
        for k, v in params.items():
            ...
            elif self.params_map[k].data_type == ParameterTypes.POSITION or self.params_map[k].data_type == ParameterTypes.XY_POSITION:
                # Treat NodeParameter with multiple values
                if self.subparam_str in k:
                    subparam_num = int(k.split(self.subparam_str)[1])
                    node_param = self.params_map[k]
    ...
    def _evaluate_function(
        ...
        if isinstance(params, list):
            outputs =  []
            for param in params:
                outputs.append(self._evaluate_single_function(param))
            return outputs
        else:
            return self._evaluate_single_function(params)
